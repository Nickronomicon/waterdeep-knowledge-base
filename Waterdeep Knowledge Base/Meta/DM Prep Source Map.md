---
type: meta
status: active
updated: 2026-07-09
source_paths:
  - ../raw/notes/discord-synopsis-channel.md
  - ../raw/notes/dm-planning/session-1/opening-scene.md
  - ../raw/notes/dm-planning/session-3/prep-notes.md
  - ../raw/notes/dm-planning/session-4/prep_sheet.md
  - ../raw/notes/dm-planning/session-4/session_4_notes.md
  - ../raw/notes/dm-planning/session-4/traversing_blackstaff_tower.md
  - ../raw/notes/dm-planning/session-4/vajra_recruitment.md
  - ../raw/notes/dm-planning/session-5/outline.md
  - ../raw/notes/dm-planning/session-5/prep_sheet.md
  - ../raw/notes/dm-planning/session-6/lightsinger_theater.md
  - ../raw/notes/dm-planning/session-6/run_sheet.md
  - ../raw/notes/dm-planning/session-7/outline.md
  - ../raw/notes/dm-planning/session-8/outline.md
  - ../raw/notes/dm-planning/session-10/outline.md
  - ../raw/notes/dm-planning/session-11/outline.md
  - ../raw/stats.md
  - ../raw/notes/technical-notes.md
tags:
  - waterdeep
  - dm-notes
  - transcript-ingest
---

# DM Prep Source Map

This map ingests the current DM-facing notes under `../raw/` as a preparation layer for transcript ingest. These notes are not table truth by themselves. They record intended scenes, private setup, clue structure, unused contingencies, and likely reveal paths.

## Ingest Rule

When a session transcript is processed, pair it with the row below. Promote only confirmed table events into session truth. Keep unplayed prep as delayed, unused, or available future material.

## Session Source Map

| Session | Prep/Summary Sources | Main Prepared Material | Transcript Ingest Focus |
|---|---|---|---|
| 1 | `../raw/notes/dm-planning/session-1/opening-scene.md`; `../raw/notes/discord-synopsis-channel.md` | Yawning Portal opener, Yagra/Krentz brawl, troll and stirges, Volo's Floon job. | Reconstruct because no transcript exists; separate opening read-aloud from synopsis of actual events. |
| 2 | `../raw/notes/discord-synopsis-channel.md`; transcript sources | Dock Ward route, Zhentarim/Xanathar street violence, Skewered Dragon, Xoblob, warehouse, Renaer/Floon confusion. | Use synopsis as a navigation aid, then verify against transcript. |
| 3 | `../raw/notes/dm-planning/session-3/prep-notes.md` | Trollskull arrival, urchins, Lif, Frewn, guild repair estimate, Threestrings/Harper pitch, Vajra sending. | Determine which Trollskull neighbors, faction contacts, costs, and repair facts actually entered play. |
| 4 | `../raw/notes/dm-planning/session-4/prep_sheet.md`; `session_4_notes.md`; `traversing_blackstaff_tower.md`; `vajra_recruitment.md` | Blackstaff Tower, Gray Hands framing, Dock Ward monster disturbance, grell/warehouse setup, return complication, opera setup. | Verify timing: same night vs next morning, what Vajra offered, what evidence was recovered, and which faction links were revealed. |
| 5 | `../raw/notes/dm-planning/session-5/outline.md`; `prep_sheet.md` | Post-grell Trollskull reset, cargo chit follow-up, Rassal's Chandlery, surviving handler, opera prep, Mirt transition. | Track whether the cargo trail, handler, and opera setup occurred in this session or slid later. |
| 6 | `../raw/notes/dm-planning/session-6/run_sheet.md`; `lightsinger_theater.md` | Lightsinger Theater, Mirt, Harper pitch, notable attendees, Cassalanters, spy, chase/ambush, Stone-related note. | Verify attendee list, whether Cassalanter loan hook appeared, whether the spy/note were caught, and who learned what. |
| 7 | `../raw/notes/dm-planning/session-7/outline.md` | Thomril & Sons Cartage, Maxeene, Davil/Yagra clue, Zhent response team with pyromancer. | Verify Maxeene facts, attack details, and any faction attribution. |
| 8 | `../raw/notes/dm-planning/session-8/outline.md` | Remallia Haventree at Ulbrinter Villa, Harper follow-up, paper bird, Fala credit note, Corvin and Miri Underbough job approach. | Track Remallia's exact role and whether Underbough infiltration began. |
| 9 | No DM-planning folder; handouts exist. | Likely Maxeene and Thalosmere archive handout material. | Use transcript plus `s9-*` handouts to place reveals and recipients. |
| 10 | `../raw/notes/dm-planning/session-10/outline.md` | Cassalanter loan terms, dinner with the Underboughs, loan signing, seance. | Verify which items occurred and what remains upcoming. |
| 11 | `../raw/notes/dm-planning/session-11/outline.md` | Dinner menu, rat interruption, Underbough hire/no-hire, Willifort/Tissina loan delivery. | Establish latest current state before session 12 prep. |

## Major Prep Threads To Track

- [[Places/Trollskull Manor|Trollskull Manor]] ownership, repair costs, staffing, haunting, and eventual opening.
- Urchins as local chorus and potential alarm system.
- Emmet Frewn as a petty local rival with later sabotage potential.
- Gray Hands/Force Grey as Vajra's official-but-irregular patronage track.
- Harpers as the subtler Renaer, Threestrings, Mirt, and Remallia track.
- Xanathar-linked monster releases as public panic or Watch-distraction assets.
- Zhentarim, Davil, Yagra, and Maxeene clues around the Grand Game.
- Cassalanter financing as both business solution and infernal/noble pressure vector.
- Underbough infiltration as tavern-staff opportunity and hidden sabotage risk.
- Stone of Golorr clue state, especially the note from the theater/ambush prep.

## Handouts Likely Paired With Session Ingest

| Session | Handout Sources | Use During Transcript Pass |
|---|---|---|
| 4 | `../raw/notes/handouts/s4-businesses-of-trollskull-alley.md` | Trollskull Alley business/place pages. |
| 6 | `../raw/notes/handouts/s6-cargo-chit.md`; `../raw/notes/handouts/s6-note-fragment.md` | Dock Ward grell/crate clue objects. |
| 7 | `../raw/notes/handouts/s7-scrap-of-note.md` | Spy/Grand Game clue object, if revealed. |
| 9 | `../raw/notes/handouts/s9-Maxeene-info.md`; `../raw/notes/handouts/s9-thalosmere-archive-report.md` | Maxeene, Remallia, Harpers, and House Thalosmere reveal tracking. |

## Transcript Pass Checklist

- Create or finalize the session page.
- Record start state and end state.
- Split actual events from planned events.
- Update involved PCs, NPCs, places, factions, objects, open threads, and dashboards.
- Mark reveal status for handouts and private information.
- Add unresolved questions and next-session hooks.
- Update [[INDEX]], [[CHANGELOG]], and this map if the source pairing changes.
